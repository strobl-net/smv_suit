use crate::{
    models::statement_of_account::{
        NewStatementOfAccount, StatementOfAccount, UpdateStatementOfAccount,
    },
    schema::{
        statement_of_accounts,
        statement_of_accounts::dsl::statement_of_accounts as statement_of_accounts_query,
    },
};
use diesel::prelude::*;

pub fn all(conn: &PgConnection) -> QueryResult<Vec<StatementOfAccount>> {
    statement_of_accounts_query
        .order(statement_of_accounts::id.asc())
        .load::<StatementOfAccount>(conn)
}
...
) -> QueryResult<StatementOfAccount> {
